...
class GetAverageRatings(APIView):
    permission_classes = [AllowAny]
    def get(self, request):
        q = "SELECT stations_stationmodel.*, CAST(feedback_responsemodel.res4 as INT), AVG(feedback_responsemodel.res4) AS count FROM feedback_responsemodel INNER JOIN stations_stationmodel ON feedback_responsemodel.station_id = stations_stationmodel.station_id GROUP BY stations_stationmodel.district"
        queryset = stationModel.objects.raw(q)
